#ifndef CLOUD_RTC_MEDIA_ENGINE_H
#define CLOUD_RTC_MEDIA_ENGINE_H
...
public:
  virtual bool onCaptureVideoFrame(VideoFrame& videoFrame) = 0;//plug in
  virtual bool onRenderVideoFrame(unsigned int uid, VideoFrame& videoFrame) = 0;//plug in
};
...
{
  IExternalVideoRenderCallback* renderCallback;//暂缓实现：cc没有用到这个回调
  void* view;
  int renderMode;
  int zOrder;
  float left;
  ...
  ExternalVideoRenerContext()
	  : renderCallback((IExternalVideoRenderCallback*)0)
	  , view((void*)0)
	  , renderMode(0)
	  , zOrder(0)
	  , left(.0f)
...
#endif //CLOUD_RTC_MEDIA_ENGINE_H
